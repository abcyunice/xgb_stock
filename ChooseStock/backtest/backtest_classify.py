...
20200826 : 将标签设为0-1。抓涨停。同时调整样本权重。将稀有的样本权重扩大，验证集将大跌的样本权重扩大。
...
def helper(array_):
    array_[array_ >= 0.06] = 1
    array_[array_ < 0.06] = 0
...
for year in range(2015, 2021):
    for month in range(1, 13):
        for i in range(1, 32):
            ...
            myxgb = xgb.XGBClassifier(n_estimators=10000, learning_rate=0.04)
            eval_set = [(df_sh_test_X, df_sh_test_y)]
            myxgb.fit(df_sh_train_X, df_sh_train_y, eval_metric="mlogloss", eval_set=eval_set, verbose=False,
            ...
            y_pred_p = myxgb.predict_proba(X)
            class_idx = np.argwhere(myxgb.classes_ == 4)[0, 0]
...
print(df_result)
df_result.to_csv("../datasets/backtest_new_0825_label.csv", index=False)
